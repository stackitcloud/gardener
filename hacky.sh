...
EFFECTIVE_VERSION=v1.50.0-ske-acl
REGISTRY=reg3.infra.ske.eu01.stackit.cloud/gardener-ds
...
ADMISSION_CONTROLLER_REPOSITORY=$REGISTRY/admission-controller
RESOURCE_MANAGER_REPOSITORY=$REGISTRY/resource-manager
...
docker build --build-arg EFFECTIVE_VERSION=$EFFECTIVE_VERSION -t $ADMISSION_CONTROLLER_REPOSITORY:$EFFECTIVE_VERSION    -f Dockerfile --target admission-controller .
docker build --build-arg EFFECTIVE_VERSION=$EFFECTIVE_VERSION -t $RESOURCE_MANAGER_REPOSITORY:$EFFECTIVE_VERSION        -f Dockerfile --target resource-manager .
...
docker push $ADMISSION_CONTROLLER_REPOSITORY:$EFFECTIVE_VERSION
docker push $RESOURCE_MANAGER_REPOSITORY:$EFFECTIVE_VERSION
