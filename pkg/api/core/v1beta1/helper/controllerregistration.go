...
// SelfHostedShootExposureEndpointUpdateEnabled returns true if the SelfHostedShootExposure controller registered for
// the given extension type opts into continuous endpoint updates. Returns false if no matching registration exists.
func SelfHostedShootExposureEndpointUpdateEnabled(registrations []gardencorev1beta1.ControllerRegistration, extensionType string) bool {
	for _, reg := range registrations {
		for _, res := range reg.Spec.Resources {
			if res.Kind == extensionsv1alpha1.SelfHostedShootExposureResource && strings.EqualFold(res.Type, extensionType) {
				return ptr.Deref(res.SelfHostedShootExposureEndpointUpdate, true)
			}
		}
	}
	return false
