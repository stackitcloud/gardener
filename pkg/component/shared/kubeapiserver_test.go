	...
	Describe("#DeployKubeAPIServer", func() {
		...
		DescribeTable("should correctly set the autoscaling apiserver resources",
			func(prepTest func(), autoscalingConfig apiserver.AutoscalingConfig, expectedResources *corev1.ResourceRequirements) {
				...
				Expect(DeployKubeAPIServer(ctx, runtimeClient, namespace, kubeAPIServer, apiServerConfig, serverCertificateConfig, sniConfig, externalHostname, externalServer, nil, nil, &nodeNetworkCIDR, etcdEncryptionKeyRotationPhase, serviceAccountKeyRotationPhase, wantScaleDown)).To(Succeed())
			},
		...
		Describe("ETCDEncryptionConfig", func() {
			...
			It("It should deploy KubeAPIServer with the default resources appended to the passed resources", func() {
				...
				Expect(DeployKubeAPIServer(ctx, runtimeClient, namespace, kubeAPIServer, apiServerConfig, serverCertificateConfig, sniConfig, externalHostname, externalServer, resourcesToEncrypt, encryptedResources, &nodeNetworkCIDR, etcdEncryptionKeyRotationPhase, serviceAccountKeyRotationPhase, wantScaleDown)).To(Succeed())
			})
		})
