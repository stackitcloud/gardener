...
func NewCacheSyncHealthz(cacheSyncWaiter cacheSyncWaiter) healthz.Checker {
	return func(_ *http.Request) error {
		// limit waiting for cache sync to 100ms, using a shorter timeout might be problematic in throttled containers
		ctx, cancel := context.WithTimeout(context.Background(), 100*time.Millisecond)
		defer cancel()

		if !cacheSyncWaiter.WaitForCacheSync(ctx) {
			return fmt.Errorf("informers not synced yet: %w", ctx.Err())
		}
		return nil
	}
}
